...
- **`materials/knowledge-base`**: 存放系统化的前端领域知识。

### 6.1 知识库驱动的问题生成策略

为了让 AI 生成的问题更具深度和专业性，`femock` 将采用一套知识库驱动的上下文增强策略。

1.  **关键词提取**:
    - 在 **`analyze`** 阶段，系统会从用户的简历和 JD 中提取核心技术关键词（如 "React Native", "微前端", "性能优化"）。
    - 在 **`dig deeper`** 阶段，系统会从用户的最新回答中提取关键词。

2.  **智能检索**:
    - 系统使用提取的关键词，在 `materials/knowledge-base` 和 `materials/algorithms` 目录下进行智能检索。
    - 检索会同时匹配文件名和文件内容，以找出与关键词最相关的知识点文档。

3.  **上下文注入**:
    - 系统会从检索到的文档中，提取出最核心的内容片段。
    - 这些片段将作为“背景知识”或“参考资料”，被动态地注入到即将发送给 AI 的 Prompt 中。

4.  **生成指令**:
    - 最终的指令会要求 AI：“请结合候选人的简历/回答，并**重点参考**以下背景知识，来设计一个面试问题/追问。”

通过这套机制，AI 的提问将不再是泛泛而谈，而是基于我们提供的专业、结构化的知识库，从而保证了面试的质量。
